...
import {
  FaArrowLeft, FaMicrophone, FaMicrophoneSlash, FaCamera, FaGamepad, FaDesktop, FaComments
} from 'react-icons/fa';
...
const StreamRoom = () => {
  ...
  const [username] = useState(sessionStorage.getItem("username") || localStorage.getItem("username") || "You");
  ...
  const spotifyMap = {
    'Self-Care & Serenity 🧘‍♀️': "https://open.spotify.com/embed/playlist/6951c68JXcMUPqLDRKIjz1?utm_source=generator",
    'Body Love 💗': "https://open.spotify.com/embed/playlist/7GOkH8QeCDuzPTlwEJ3zeR?utm_source=generator",
    'K-pop Crush 🎶': "https://open.spotify.com/embed/playlist/00kQrnQz1XKXhsd8xBWxdi?utm_source=generator",
    'K-Drama Feels 🎬': "https://open.spotify.com/embed/playlist/37i9dQZF1DWUXxc8Mc6MmJ?utm_source=generator",
    'Broken Bonds 💔': "https://open.spotify.com/embed/playlist/37i9dQZF1DWWbVYL1LBbVy?utm_source=generator",
    'Solo Escapes ✈️': "https://open.spotify.com/embed/playlist/3tKyCWqLHaVGoiVzoPe68N?utm_source=generator",
    'Swipe Stories 📱': "https://open.spotify.com/embed/playlist/0E4oCknVXU53DMhdl7DkZA?utm_source=generator",
  ...
  const gamesMap = {
    'Self-Care & Serenity 🧘‍♀️': [
      { name: "Hashiwokakero", url: "https://cdn.htmlgames.com/Hashiwokakero/" },
      { name: "Crossword", url: "https://cdn.htmlgames.com/DailyCrossword/" }
    ],
    'Body Love 💗': [
      { name: "Maya Golf", url: "https://cdn.htmlgames.com/MayaGolf2/" },
      { name: "Bubble Monster", url: "https://cdn.htmlgames.com/BubbleMonster/" }
    ],
    'K-pop Crush 🎶': [
      { name: "Crack The Code", url: "https://cdn.htmlgames.com/CrackTheCode/" },
    ...
    'Broken Bonds 💔': [
      ...
      { name: "Japanese Garden", url: "https://cdn.htmlgames.com/JapaneseGarden-HiddenSecrets/" }
    ],
    'Solo Escapes ✈️': [
      { name: "Hidden Spots-City", url: "https://cdn.htmlgames.com/HiddenSpotsCity/" },
    ...
    'Hustle & Heart 💼': [
      ...
      { name: "Speed Directions", url: "https://cdn.htmlgames.com/SpeedDirections/" }
    ],
    '2025 Fashion Files 👗': [
      { name: "Mohjongg Pyraminds", url: "https://cdn.htmlgames.com/MahjonggPyramids/" },
      { name: "Jigsaw Cities", url: "https://cdn.htmlgames.com/JigsawCities1/" }
    ],
    'Glow Up Goals 🌟': [
  ...
  useEffect(() => {
    ...
    socket.on('user-left', ({ userId, username: leftName }) => {
      ...
      setParticipantMap(prev => { const p = { ...prev }; delete p[userId]; return p; });
      if (remoteScreenSharer === userId) setRemoteScreenSharer(null);
      // Small toast notification for participant leaving
      try { window?.toast?.info?.(`${leftName || 'Someone'} left the room`); } catch {}
      if (currentHostId === userId) {
        // keep UI in neutral state until host-left arrives
        setCurrentHostId(null);
      }
    });

    socket.on('group-peers', ({ peers: existingPeers, peerDetails = [], hostId, self }) => {
      console.log('Existing peers in room:', existingPeers, 'host:', hostId);
      setParticipants(prev => Array.from(new Set([...prev, ...existingPeers])));
      // Build name map (include self)
      setParticipantMap(prev => {
        const map = { ...prev };
        if (self?.userId && self?.username) map[self.userId] = self.username;
        peerDetails.forEach(p => { if (p.userId) map[p.userId] = p.username || 'User'; });
        return map;
      });
    ...
    // Listen for chat messages (clear previous listener to avoid duplicates)
    ...
    socket.on('stream-message', (payload) => {
      ...
      setChatMessages(prev => [...prev, { from, text, timestamp: Date.now(), rawFrom, userId }]);
    });

    // Add your own message locally for immediate UX
    // Note: server already echoes back to sender; this ensures consistency if server ever changes
    const addOwnMessage = (text) => {
      setChatMessages(prev => [...prev, { from: username, text, timestamp: Date.now() }]);
    };
    ...
    socket.on('host-left', ({ previousHostId, candidates }) => {
      // Show prompt to continue and then choose new host
      setShowHostPrompt(true);
      setShowElection(false);
      setHostCandidates(candidates || []);
    });

    // Listen for tab changes from other users
    socket.on('tab-changed', ({ userId, activeTab, data, username }) => {
      console.log(`${username} changed tab to:`, activeTab, data);
      // Sync active tab and game selection so everyone sees the same content
      setActiveTab(activeTab);
      if (activeTab === 'games' && typeof (data?.selectedGameIndex) === 'number') {
        setSelectedGameIndex(data.selectedGameIndex);
      }
    });

    // Listen for game state updates
    socket.on('game-state-update', ({ userId, gameType, gameState, username }) => {
      console.log(`${username} updated ${gameType}:`, gameState);
      // Sync game state if needed
    });

    // Listen for Spotify updates
    socket.on('spotify-state-update', ({ userId, isPlaying, track, position, username }) => {
  ...
  useEffect(() => {
    if (cameraVideoRef.current && cameraStream) {
      ...
      cameraVideoRef.current.onloadedmetadata = () => {
        cameraVideoRef.current.play().catch(e => console.error('Error playing local camera:', e));
      };
    }
  }, [cameraStream]);
  ...
  useEffect(() => {
    ...
    const remoteStream = remoteScreenSharer ? remoteStreams[remoteScreenSharer] : null;
    ...
    if (chosen) {
      videoEl.onloadedmetadata = () => {
        videoEl.play().catch(e => console.error('Error playing screen video:', e));
      };
    }
  ...
  useEffect(() => {
    ...
    Object.entries(remoteStreams).forEach(([userId, stream]) => {
      if (stream) {
        console.log(`Remote stream for ${userId}:`, stream.getTracks().map(t => ({ kind: t.kind, enabled: t.enabled, readyState: t.readyState })));
      }
    });
  }, [remoteStreams]);
    ...
    try {
      if (cameraOn) {
        ...
        if (localStreams.current.camera) {
          localStreams.current.camera.getTracks().forEach(track => track.stop());
  ...
  const toggleScreenShare = async () => {
    try {
      if (isScreenSharing) {
        ...
        socket.emit('screen-share-state', { roomId: roomCode, isSharing: false });
      } else {
        ...
        socket.emit('screen-share-state', { roomId: roomCode, isSharing: true });
        ...
        Object.entries(peers.current).forEach(([peerId, pc]) => {
          ...
          pc.createOffer()
            ...
            .catch(err => console.error('Error creating offer after adding screen tracks:', err));
        });
      }
    } catch (error) {
          ...
          if (audioTrack) {
            ...
            Object.entries(peers.current).forEach(([peerId, pc]) => {
              ...
              pc.createOffer()
                .then(offer => pc.setLocalDescription(offer))
                .then(() => {
                  socket.emit('signal', { to: peerId, signal: { type: 'offer', offer: pc.localDescription } });
                })
                .catch(err => console.error('Error creating offer after adding mic track:', err));
            });
            track = audioTrack;
          }
        }
        ...
        if (!micOn) {
          ...
          if (audioTrack) {
            ...
            Object.entries(peers.current).forEach(([peerId, pc]) => {
              ...
              pc.createOffer()
                ...
                .catch(err => console.error('Error creating offer after adding standalone mic:', err));
            });
          }
        } else {
          ...
          if (micStream) {
            ...
            Object.values(peers.current).forEach(pc => {
              pc.getSenders()
                ...
                .forEach(sender => {
                  try { pc.removeTrack(sender); } catch (e) { console.warn('removeTrack failed', e); }
                });
            });
            // Stop tracks
          ...
          Object.entries(peers.current).forEach(([peerId, pc]) => {
            pc.createOffer()
              ...
              .catch(err => console.error('Error creating offer after removing mic:', err));
          });
        }
      }
  ...
  const handleEmojiClick = (emojiData) => { setChatInput(chatInput + emojiData.emoji); setShowEmojiPicker(false); };
  ...
  return (
    ...
    >
      <div className="stream-header">
        <button className="back-btn" onClick={() => navigate('/room')}><FaArrowLeft /></button>
        <div className="room-info">
          <div className="room-details">
            <span className="room-name">{roomName}</span>
            <span className="room-code">Room Code: {roomCode}</span>
            <span className="participants-count">{new Set([...participants, socket.id]).size} participants</span>
            {currentHostId === socket.id && <span className="host-badge">Host</span>}
          </div>
        </div>
        <button className="game-nav-btn" onClick={() => setShowParticipantsPanel(v => !v)}>Participants</button>
      </div>
        ...
        <div className="overlay-tab">
          ...
          {activeTab === "games" && (
            <>
              ...
              <iframe src={currentGames[selectedGameIndex].url} title={currentGames[selectedGameIndex].name} width="100%" height="100%" frameBorder="0" style={{ flex: 1 }} />
            </>
          )}

          {activeTab === "spotify" && (
            <iframe src={spotifyMap[roomName] || spotifyMap.Default} title="Spotify" frameBorder="0" allow="autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture" width="100%" height="100%" />
          )}

          {activeTab === "camera" && cameraStream && (
            <video className="camera-feed" ref={cameraVideoRef} autoPlay playsInline muted style={{ width: "100%", height: "100%", objectFit: "cover" }} />
          )}

          {activeTab === "screen" && screenStream && (
            <video ref={screenVideoRef} autoPlay playsInline style={{ width: "100%", height: "100%", objectFit: "cover" }} />
          )}
        </div>
      )}
      ...
      {showParticipantsPanel && (
        <div className="overlay-tab" style={{ width: 380, height: 'auto' }}>
          <button className="close-tab" onClick={() => setShowParticipantsPanel(false)}>×</button>
          <div style={{ padding: 16 }}>
            <h3 style={{ margin: '0 0 10px' }}>Participants</h3>
            <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'grid', gap: 8 }}>
              {[socket.id, ...participants].filter((v,i,a)=>a.indexOf(v)===i).map(pid => (
                <li key={pid} style={{ display: 'flex', alignItems: 'center', background: '#f8f9fa', border: '1px solid #e9ecef', borderRadius: 8, padding: '8px 10px' }}>
                  <span>
                    {participantMap[pid] || (pid === socket.id ? 'You' : 'User')}
                    {pid === currentHostId && <span className="host-badge" style={{ marginLeft: 8 }}>Host</span>}
                  </span>
                </li>
              ))}
            </ul>
      ...
      {(isScreenSharing && screenStream) || (remoteScreenSharer && remoteStreams[remoteScreenSharer]) ? (
      ...
      {isChatOpen && (
        <div className="chat-panel">
          ...
          {showEmojiPicker && <div className="emoji-picker-container"><EmojiPicker onEmojiClick={handleEmojiClick} height={300} width={280} /></div>}
          <div className="chat-input-container">
            <input value={chatInput} onChange={(e) => setChatInput(e.target.value)} className="chat-input" placeholder="Type a message..." onKeyPress={(e) => e.key === 'Enter' && sendMessage()} />
      ...
      {showHostPrompt && (
        <>
          ...
          <div className="overlay-tab" style={{ width: 420, height: 'auto', paddingBottom: 12 }}>
            <button className="close-tab" onClick={() => setShowHostPrompt(false)}>×</button>
            <div style={{ padding: 20 }}>
              <h3 style={{ marginTop: 0 }}>Host left the room</h3>
              <p>Do you want to continue?</p>
              <div style={{ display: 'flex', gap: 10, margin: '10px 0 20px' }}>
                <button className="game-nav-btn" onClick={() => { socket.emit('host-continue', { roomId: roomCode }); setShowElection(true); }}>Yes</button>
                <button className="game-nav-btn" onClick={() => { setShowHostPrompt(false); navigate('/room'); }}>No</button>
              </div>
                ...
                <>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                    {hostCandidates.map(c => (
                      <button key={c.userId} className="game-nav-btn" onClick={() => { socket.emit('host-set', { roomId: roomCode, newHostId: c.userId }); setShowHostPrompt(false); setShowElection(false); }}>
                        {c.username}
                      </button>
                    ))}
                  </div>
      ...
      <div className="stream-controls">
        <SiSpotify title="Spotify" onClick={() => toggleTab('spotify')} className={activeTab === 'spotify' ? 'active-control' : ''} />
        <FaCamera title="Camera" onClick={toggleCamera} className={cameraOn ? 'active-control' : ''} />
        {micOn
          ? <FaMicrophone title="Mic On" onClick={toggleMic} className="mic-on active-control" />
          : <FaMicrophoneSlash title="Mic Off" onClick={toggleMic} />}
        <FaComments title="Chat" onClick={toggleChat} className={isChatOpen ? 'active-control' : ''} />
        <FaGamepad title="Games" onClick={() => toggleTab('games')} className={activeTab === 'games' ? 'active-control' : ''} />
        <FaDesktop title="Share Screen" onClick={toggleScreenShare} className={isScreenSharing ? 'active-control' : ''} />
      </div>
    </div>
  );
};
